...
exports.changeDeliveryStatus = async (req, res) => {
  try {
    ...
    if (req.body.status === 'DELIVERED') {
      ...
      const listing = await catalogModel.getListingBasic(match.listing_id);
      console.log("listing basic:", listing);
      if (!listing) {
        return res.status(500).json({ message: 'Listing not found for this match' });
      }

      const inventoryItemId = listing.inventory_item_id;
      const amount = Number(request.quantity || 1);

      const [r] = await db.query(
        `UPDATE inventory_items
   SET quantity = quantity - ?
   WHERE id = ? AND quantity >= ?`,
        [amount, inventoryItemId, amount]
      );
      console.log("affectedRows:", r.affectedRows);
      if (r.affectedRows === 0) {
        return res.status(400).json({ message: 'Not enough stock in inventory' });
      }
      await shortageRepo.checkAndCreateForInventoryItem(inventoryItemId);
      return res.status(200).json({
